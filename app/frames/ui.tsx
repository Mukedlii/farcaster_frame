...
import type { GamePage, GameState } from "./types";
import { isBoostActive } from "./state";

const formatPoints = (points: number) => `${points.toLocaleString("en-US")} pts`;

export const renderImage = (state: GameState) => {
  const accent = "#f43f5e";
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", gap: 24, marginTop: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 26, fontWeight: 700 }}>{formatPoints(state.points)}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div style={{ fontSize: 18, fontWeight: 600, color: accent }}>$PANIC</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ fontSize: 12, color: "#94a3b8" }}>Page: {state.page.toUpperCase()}</div>
      </div>
    </div>
  );
};

export const renderButtons = (page: GamePage) => {
  if (page === "home") {
    return [
